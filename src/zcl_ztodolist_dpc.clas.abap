class ZCL_ZTODOLIST_DPC definition
  public
...
protected section.
  ...
  methods TODOSET_CREATE_ENTITY
    ...
    exporting
      !ER_ENTITY type ZCL_ZTODOLIST_MPC=>TS_TODO
    raising
  ...
  methods TODOSET_GET_ENTITYSET
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZTODOLIST_MPC=>TT_TODO
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods TODOSET_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
CLASS ZCL_ZTODOLIST_DPC IMPLEMENTATION.
...
*&* This class has been generated on 05.06.2022 17:19:20 in client 001
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZTODOLIST_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA todoset_create_entity TYPE zcl_ztodolist_mpc=>ts_todo.
...
*     Call the entity set generated method
    todoset_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
       	 ...
       	 IMPORTING er_entity          = todoset_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = todoset_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA todoset_get_entity TYPE zcl_ztodolist_mpc=>ts_todo.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          todoset_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = todoset_get_entity
                         es_response_context = es_response_context
          ).

        IF todoset_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = todoset_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZTODOLIST_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA todoset_get_entityset TYPE zcl_ztodolist_mpc=>tt_todo.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
      todoset_get_entityset(
        EXPORTING
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = todoset_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = todoset_get_entityset
        CHANGING
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA todoset_update_entity TYPE zcl_ztodolist_mpc=>ts_todo.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*     Call the entity set generated method
          todoset_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = todoset_update_entity
          ).
       IF todoset_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = todoset_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Unit test injection
  ...
  method IF_SADL_GW_DPC_UTIL~GET_DPC.
    TYPES ty_z_i_todo_1 TYPE z_i_todo ##NEEDED. " reference for where-used list

    DATA(lv_sadl_xml) =
               |<?xml version="1.0" encoding="utf-16"?>| &
               |<sadl:definition xmlns:sadl="http://sap.com/sap.nw.f.sadl" syntaxVersion="V2" >| &
               | <sadl:dataSource type="CDS" name="TODOSet" binding="Z_I_TODO" />| &
               |<sadl:resultSet>| &
               |<sadl:structure name="TODOSet" dataSource="TODOSet" maxEditMode="EX" >| &
               ...
               | <sadl:attribute name="ENDTIME" binding="ENDTIME" isOutput="TRUE" isKey="FALSE" />| &
               |</sadl:structure>| &
               |</sadl:resultSet>| &
               |</sadl:definition>| .
    ro_dpc = cl_sadl_gw_dpc_factory=>create_for_sadl( iv_sadl_xml   = lv_sadl_xml
               iv_timestamp         = 20220605211924
               iv_uuid              = 'ZTODOLIST'
               io_query_control     = me
